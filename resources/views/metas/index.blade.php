...
<p class="text-secondary mb-4">Mira cuánto llevas y cuándo puedes lograrlo.</p>
...
<form method="POST" action="{{ route('app.metas.store') }}" class="card border-0 shadow-sm rounded-4 p-3 mb-4 vstack gap-3">@csrf
    ...
    <div class="row g-3"><div class="col-6"><label class="form-label">Objetivo</label><input name="objetivo" data-miles inputmode="decimal" class="form-control" required></div><div class="col-6"><label class="form-label">Monto actual</label><input name="monto_actual" data-miles inputmode="decimal" class="form-control" value="0"></div></div>
    <div class="row g-3"><div class="col-6"><label class="form-label">Fecha objetivo</label><input name="fecha_objetivo" type="date" class="form-control"></div><div class="col-6"><label class="form-label">Ahorro mensual</label><input name="aporte_mensual" data-miles inputmode="decimal" class="form-control" value="0"></div></div>
    <select name="cuenta_liquida_id" class="form-select"><option value="">Cuenta asociada (opcional)</option>@foreach($cuentas as $cuenta)<option value="{{ $cuenta->id }}">{{ $cuenta->nombre }}</option>@endforeach</select>
